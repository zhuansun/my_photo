...
//----------------------------UPLOADFILES------------------------------------
    ...
    Ssi_upload.prototype.clear = function (action) {
        //clean the list of all non in progress files
        console.log("清空")
    ...
    Ssi_upload.prototype.uploadFiles = function () {// upload the pending files
        ...
        function ajaxLoopRequest(formData, ii) {
            ...
            $.ajax(ajaxOptions).done(function (responseData, textStatus, jqXHR) {
                ...
                if (thisS.options.responseValidation) {
                    ...
                    if (typeof valData.validationKey === 'object' && valData.resultKey == 'validationKey') {
                        if (data.hasOwnProperty(valData.validationKey.success)) {
                            ...
                            cb(false, data[valData.validationKey.error], data);
                        }
                    } else {
                        if (data[valData.validationKey] == valData.success) {
                            cb(true, null, data);
                        } else {
                            cb(false, data[valData.resultKey], data);
                        }
                    }
                } else {
                    if (jqXHR.status == 200) {
                ...
                function cb(result, data, responseData) {
                    if (result) {//if response type is success
                        ...
                        //先保存div中原来的html
                        var html = document.getElementById("successResult").innerHTML;
                        //再跟你想追加的代码加到一起插入div中
                        document.getElementById("successResult").innerHTML = html + "<span>" + responseData + "</span></p>";
    ...
    var locale = {
        ...
        zh: {
            ...
            clear: '清空',
            drag: '拖拽图片到此处',
            sizeError: '$1 超过 $2 大小限制',// $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
